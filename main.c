...
int main()
{
    ...
    if(Database_Init(&usersDatabase, 30)==0) {
       printf("!");
       exit(-1);
   } else
    ...
    while(1){
        ...
        case 1:
            ...
            Database_AddUser(&usersDatabase, user);
            break;
        case 2:
            printf("Enter user index to delete: ");
            scanf("%d",&index);
            if (Database_RemoveUser(&usersDatabase, index)==0)
                printf("!");
            else
                printf("Deleted user.");
            break;
        case 3:
            ...
            bool result;
            a = Database_GetUser(&usersDatabase, index, &result);
